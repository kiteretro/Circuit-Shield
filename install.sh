...
if [[ ! $(grep "^uinput$" $DEST/etc/modules) ]] ; then
  execute "echo 'uinput' >> $DEST/etc/modules"
fi
if [[ ! $(grep "^i2c-dev$" $DEST/etc/modules) ]] ; then
  execute "echo 'i2c-dev' >> $DEST/etc/modules"
fi
